...
#store the id that has been used
...
path = "samples"
print(face_id)
r_path = os.path.join(path, face_id)
os.mkdir(r_path)
...
while True:
    ...
    for (x, y, w, h) in faces:
        ...
        #cv2.imwrite(r_path+"face." + 1 + '.' + str(count) + ".jpg", converted_image[y:y + h, x:x + w])
        cv2.imwrite(os.path.join(r_path,"face." + "1" + '.' + str(count) + ".jpg"), converted_image[y:y + h, x:x + w])
